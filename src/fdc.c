...
#if 0	/* ReadOnly でファイルを開いた場合、無条件でライトプロテクト状態と
	   していたが、ライトプロテクトシールをチェックするゲームの場合、
	   先に進まなくなる。(エメドラ、ミスティブルーなど)
	   なので、 ReadOnly かどうかによらず、ライトプロテクト状態は
	   イメージの属性をそのまま使うことにしよう。当然この場合は、
	   ライト時にはじめてエラーになる。
	   実際のところ、どっちにするのが便利なのかな？ */
...
static	int	fdc_write_data( void )
{
  ...
  int	write_size, total_size, size, ptr, error=0;
  int	drv = fdc.us;
  unsigned char	c[2];
  ...
  print_fdc_status(BP_WRITE, drv, drive[drv].track, drive[drv].sec);
	...
	/* 処理の最中にディスクが交換されたりした時は、処理せず正常終了 */
  ...
  if( disk_not_exist( drv ) ||				/* ディスク無し */
    ...
    if( verbose_fdc )
      printf("FDC %s : Drive %d Image Error (file changed ?)\n",
  ...
  while( write_size > 0 ){	/* -------------------指定サイズ分書き続ける */
    ...
    total_size += (sec_buf.size + SZ_DISK_ID);	/* サイズが増えた     */
...
static	int	fdc_write_id( void )
{
  ...
  if( disk_not_exist( drv ) ||				/* ディスク無し */
	...
	printf("FDC %s : Drive %d Can't Format (no file space)\n",
	       cmd_name[ fdc.command ], drv+1);
      else
	printf("FDC %s : Drive %d Image Error (file changed ?)\n",
	       cmd_name[ fdc.command ], drv+1);
...
{
  int cmd, nd;
  static unsigned char c0, c1, ncn, s0, s1;
  unsigned char * const table[][10] =
  {
/*WAIT */{ 0 },
/*R_DAT*/{ &c0,&c1, &fdc.c,&fdc.h,&fdc.r,&fdc.n, &fdc.eot,&fdc.gpl,&fdc.dtl,0},
/*R_DEL*/{ &c0,&c1, &fdc.c,&fdc.h,&fdc.r,&fdc.n, &fdc.eot,&fdc.gpl,&fdc.dtl,0},
/*R_DIA*/{ &c0,&c1, &fdc.c,&fdc.h,&fdc.r,&fdc.n, &fdc.eot,&fdc.gpl,&fdc.dtl,0},
/*R_ID */{ &c0,&c1, 0 },
/*W_DAT*/{ &c0,&c1, &fdc.c,&fdc.h,&fdc.r,&fdc.n, &fdc.eot,&fdc.gpl,&fdc.dtl,0},
/*W_DEL*/{ &c0,&c1, &fdc.c,&fdc.h,&fdc.r,&fdc.n, &fdc.eot,&fdc.gpl,&fdc.dtl,0},
/*W_ID */{ &c0,&c1, &fdc.n, &fdc.sc, &fdc.gpl, &fdc.d, 0 },
/*S_EQU*/{ &c0,&c1, &fdc.c,&fdc.h,&fdc.r,&fdc.n, &fdc.eot,&fdc.gpl,&fdc.stp,0},
/*S_LOW*/{ &c0,&c1, &fdc.c,&fdc.h,&fdc.r,&fdc.n, &fdc.eot,&fdc.gpl,&fdc.stp,0},
/*S_HIG*/{ &c0,&c1, &fdc.c,&fdc.h,&fdc.r,&fdc.n, &fdc.eot,&fdc.gpl,&fdc.stp,0},
/*SEEK */{ &c0,&c1, &ncn, 0 },
/*RECAL*/{ &c0,&c1,       0 },
/*SNS_I*/{ &c0,     0 },
/*SNS_D*/{ &c0,&c1, 0 },
  ...
  if( fdc.status & REQ_MASTER ){	/* ホストからのデータ無し */
    ...
    *table[ fdc.command ][ fdc.step ] = fdc.write;		/*データ引取*/
    ...
    if( table[ fdc.command ][ fdc.step ] ){	/* まだデータが必要な時 */
      ...
      fdc.us  = (c1 & 0x03);				/* SENSE INTとINVALID*/
      fdc.hd  = (c1 & 0x04) >> 2;			/* では不要なんだけど*/
	...
	break;
      ...
      case SEEK:						/* SEEK系 */
      case RECALIBRATE:
	fdc.ncn[ fdc.us ] = (fdc.command==SEEK) ? ncn : 0 ;
	fdc.status = (fdc.status&0x0f) | FDC_BUSY;
	fdc.phase  = E_PHASE;

	logfdc("%s us%d %02d...%02d(Tr.%02d)\n", 
	       cmd_name[fdc.command],
	       fdc.us,fdc.pcn[fdc.us],fdc.ncn[fdc.us],fdc.ncn[fdc.us]*2);
	break;

      case SENSE_INT_STATUS:					/* SENSE系 */
	fdc_cancel_interrupt();    /* FALLTHROUGH */
	...
	fdc.srt_clk = (16-((s0>>4)&0x0f)) *  (2*4000);	/* (16-srt)x2ms */
	fdc.hut_clk =      (s0    &0x0f)  * (32*4000);	/*   hut x 32ms */
	fdc.hlt_clk =     ((s1>>1)&0x7f)  *  (4*4000);	/*   hlt x  4ms */
	nd          =       s1    &   1;
	if( fdc.hut_clk==0 ) fdc.hut_clk = 16 * (32*4000);		/* ? */

	logfdc("%s srt%dms hut%dms hlt%dms (%02X %02X)\n",
	       cmd_name[fdc.command],
	       fdc.srt_clk/4000,fdc.hut_clk/4000,fdc.hlt_clk/4000,s0,s1);
...
/*  ICOUNT( 0 );*/
    ...
    case SENSE_INT_STATUS:			/* SENSE INT STATUS          */
      for( i=0; i<MAX_DRIVE; i++ ){			/* シーク完(割込発生)*/
	if( fdc.seek_stat[i]==SEEK_STAT_INTR ){		/* のドライブを検索  */
	  fdc.seek_stat[i] = SEEK_STAT_STOP;
	  fdc.status &= ~(1<<i);
	  break;
	}
      }
      if( i < MAX_DRIVE ){				/* ドライブ見つけた  */
	if( i<NR_DRIVE ) st0 = ST0_IC_NT | ST0_SE | i;
	else             st0 = ST0_IC_AT | ST0_SE | ST0_NR | i;
	pcn = fdc.pcn[ i ];
      }else{						/* 見つからない時は  */
